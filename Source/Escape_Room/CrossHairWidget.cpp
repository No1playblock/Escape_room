...
void UCrossHairWidget::SubTitle1()
{
	StartSubtitles(3, 5);
}
void UCrossHairWidget::SubTitle1_2()
{
	StartSubtitles(5, 7);
}
void UCrossHairWidget::SubTitle2()
{
	StartSubtitles(7, 13);
}
void UCrossHairWidget::SubTitle3()
{
	StartSubtitles(13, 14);
}
void UCrossHairWidget::SubTitle4()
{
	UGameplayStatics::GetAllActorsWithTag(GetWorld(), FName(TEXT("RedLight")), RedLight);
...
void UCrossHairWidget::SubTitle5()
{
	UGameplayStatics::GetAllActorsWithTag(GetWorld(), FName(TEXT("RedLight")), RedLight);
	for (AActor* Actor : RedLight)
	{
...
void UCrossHairWidget::SubTitle6()
{
	UGameplayStatics::GetAllActorsWithTag(GetWorld(), FName(TEXT("RedLight")), RedLight);
	for (AActor* Actor : RedLight)
	{
...
void UCrossHairWidget::SubTitle7()
{
...
void UCrossHairWidget::DisplayNextSubtitle()
{
	if (CurrentIndex >= EndIndex)
	{
		...
		else if (CurrentIndex == 13)	//subtitle2
		{
			...
			player->CurrentMap = ECharacterMapState::Map2;
			player->CanShoot = false;
			GetWorld()->GetTimerManager().SetTimer(SubtitleTimerHandle, this, &UCrossHairWidget::SubTitle3, 6.0f, false);
			//SubTitle3();
		}
...
void UCrossHairWidget::LastLoop()
{
	//USoundBase* SoundToPlay = LoadObject<USoundBase>(nullptr, TEXT("/Game/PathToYourSound/Map2_WarpSound.Map2_WarpSound"));
	if (warpsound)
	{
	...
	GetWorld()->GetTimerManager().SetTimer(WarpTimerHandle, this, &UCrossHairWidget::Map3warp, 5.0f, false);
}

void UCrossHairWidget::Map3warp()
{
	...
	AEscape_RoomCharacter* player = Cast<AEscape_RoomCharacter>(GetOwningPlayerPawn());
	player->CurrentMap = ECharacterMapState::Map3;
