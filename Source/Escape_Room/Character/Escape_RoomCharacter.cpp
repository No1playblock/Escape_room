...
#include "Prop/Drawer.h"
#include "Prop/Bed.h"
#include "Prop/Arrow.h"
#include "UI/CrossHairWidget.h"
#include "UI/PointerWidget.h"
...
void AEscape_RoomCharacter::KilZombie()
{
	...
	if (ZombieKillCount == 4)
	{
		...
		Cast<UCrossHairWidget>(WidgetInstance)->ShowSubTitleAfterZombieKill();
		if (WidgetInstance)
		{
...
void AEscape_RoomCharacter::ShootArrow()
{
	...
	if (bScreenToWorld)
	{
		...
		AArrow* InArrow = GetWorld()->SpawnActor<AArrow>(Arrow, BowPosition->GetComponentLocation(), Direction.Rotation());

		InArrow->GetProjectileMovement()->Velocity = Direction.GetSafeNormal() * ArrowSpeed * 2000.0f;
	}
...
void AEscape_RoomCharacter::SetWidget() {
	...
	if (GetHasBow())
	{
		if (CrossHairWidget)
		{
			...
			Cast<UCrossHairWidget>(WidgetInstance)->ShowSubtitleAfterBowAcquired();
		}
	}
	else
	{
